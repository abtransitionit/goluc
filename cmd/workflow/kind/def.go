...
func init() {
	...
	wkf, err = corephase.NewWorkflowFromPhases(
		...
		corephase.NewPhase("installGoCli", "provision Go CLI(s).", taskgocli.InstallGoCliOnVm(listGoCli, targets), []string{"updateApp"}),
		...
		corephase.NewPhase("startOsService", "start OS services needed by the app", oservice.StartOsService(listOsService), []string{"setEnvar"}),
	)
	if err != nil {
